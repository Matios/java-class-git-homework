...
public class Main {
  private static School generateSchool() {
    ...
    school.addStudent(new Student ("Matios", "Dinegdia"));
    school.addStudent(new Student ("Indrias", "Beyene"));

    return school;
  }

  private static void createHandlers()throws NumberFormatException {
    ...
    put("/api/students/:id/grade", (request, response) -> {
      ...
      int id = Integer.parseInt(request.params(":id"));
        ...
        Student studentToUpdate= school.getStudentById(id);//added
       try {
